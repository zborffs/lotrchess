...
class Board {
    /// Member Variables
    PositionKey key_; // zobrist hash key of current position
    std::array<Bitboard, 14> piece_bb_; // piece bitboards
    Bitboard occupied_bb_; // occupied piece bitboards
    Bitboard empty_bb_; // empty piece bitboards
    std::array<Bitboard, 2> capturable_; // (all pieces &~ kingbb)
    Depth fif_move_counter_; // fifty move counter
    Square_t ep_sq_; // en passant square
    CastleRights_t castle_perm_; // castling permissions
    ...
    MoveHistory move_history_; // the move history, so that we can undo moves
...
public:
    ...
    void add_cap_move(const Square_t from_sq, const Square_t to_sq, const PieceType_t captured,
    ...
    void add_pro_move(const Square_t from_sq, const Square_t to_sq, const PieceType_t captured,
    ...
    friend std::ostream& operator<<(std::ostream &os, Board &board) {
        ...
        os << std::endl << "Fifty Move Counter: " << static_cast<int>(board.fif_move_counter()) << std::endl;
        ...
        if (board.move_history_.size() >= 1) {
            ...
            move_history_copy.reserve(board.move_history_.size() - 1);
            os << board.move_ << " ";
            ...
            for (int k = static_cast<int>(move_history_copy.size()) - 1; k >= 0; k--) {
                board.move_history_.push(move_history_copy[static_cast<unsigned long>(k)]);
            }
        } else {
            os << "Nothing";
        }
