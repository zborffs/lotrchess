...
Bitboard north_fill(Bitboard bb) {
  bb |= (bb << static_cast<unsigned>(8));
  bb |= (bb << static_cast<unsigned>(16));
  bb |= (bb << static_cast<unsigned>(32));
  return bb;
}
...
Bitboard reverse(Bitboard bb) {
  for (int i = 0; i < 6; i++) {
    int s(1 << i);
    Bitboard f(static_cast<Bitboard>(1) << s);
    Bitboard k(static_cast<Bitboard>(-1) / (f + 1) );
    bb = ((bb >> s) & k) + f * (bb & k);
  }
  return bb;
}
