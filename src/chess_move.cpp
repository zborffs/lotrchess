 ...
 */
ChessMove::ChessMove() : from_sq(A1), mf(NO_MOVE_FLAG), to_sq(A1), pf(NO_MOVE_FLAG), moved(NO_PIECE),
                         captured(NO_PIECE), score(0) {
...
ChessMove::ChessMove(Square_t from_sq, Square_t to_sq, MoveFlag_t flag, PieceType_t moved, PieceType_t captured) :
        from_sq(from_sq), mf(flag & static_cast<MoveFlag_t>(3)), to_sq(to_sq), pf(flag >> static_cast<MoveFlag_t>(2)),
        moved(moved), captured(captured), score(0) {
...
MoveFlag_t ChessMove::flag() noexcept { // NOLINT
    return static_cast<MoveFlag_t>(mf | (pf << 2)); // NOLINT
}
...
bool ChessMove::operator==(const ChessMove &rhs) const {
    return from_sq == rhs.from_sq && to_sq == rhs.to_sq && mf == rhs.mf && pf == rhs.pf && moved == rhs.moved &&
