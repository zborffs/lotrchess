 ...
 */
    void gen_pawn_cap(Board &board, std::vector<ChessMove> &movelist) {
        ...
        if (board.side_2_move() == WHITE) {
            push_pawn =
                    (board.piece_bb(W_PAWN) << 9) & board.capturable(BLACK) & ~(RANK_MASK[RANK8] | FILE_MASK[FILEA]);
            while (push_pawn) {
            ...
            push_pawn =
                    (board.piece_bb(W_PAWN) << 7) & (board.capturable(BLACK)) & ~(RANK_MASK[RANK8] | FILE_MASK[FILEH]);
            while (push_pawn) {
                to_sq = static_cast<Square_t>(bitscan_forward(push_pawn));
                ptc = board.piece_type(to_sq, BLACK);
            ...
            push_pawn =
                    (board.piece_bb(B_PAWN) >> 7) & (board.capturable(WHITE)) & ~(RANK_MASK[RANK1] | FILE_MASK[FILEA]);
            while (push_pawn) {
                to_sq = static_cast<Square_t>(bitscan_forward(push_pawn));
            ...
            push_pawn =
                    (board.piece_bb(B_PAWN) >> 9) & (board.capturable(WHITE)) & ~(RANK_MASK[RANK1] | FILE_MASK[FILEH]);
            while (push_pawn) {
                to_sq = static_cast<Square_t>(bitscan_forward(push_pawn));
                ptc = board.piece_type(to_sq, WHITE);
        ...
        if (board.side_2_move() == WHITE) {
            ...
            while (knight_bb) {
                ...
                push_knight = knight_moves(C64(1) << from_sq) &
                              board.empty_bb(); // shouldn't i just use the empty bitboard rather than the occupied one?; save 1 clock cycle
        ...
        if (board.side_2_move() == WHITE) {
            if ((board.castle_perm() & WKC)) {
                ...
                if ((board.empty_bb() & bb) == bb) { // 96 = 2^5 + 2^6
                    if (!board.is_sq_attacked(E1, BLACK) && !board.is_sq_attacked(F1, BLACK) &&
                        !board.is_sq_attacked(G1, BLACK)) {
            ...
            if ((board.castle_perm() & WQC)) {
                ...
                if ((board.empty_bb() & bb) == bb) { // 2^1 + 2^2 + 2^3
                    if (!board.is_sq_attacked(E1, BLACK) && !board.is_sq_attacked(D1, BLACK) &&
                        !board.is_sq_attacked(C1, BLACK)) {
            ...
            if ((board.castle_perm() & BKC)) {
                ...
                if ((bb & board.empty_bb()) == bb) {
                    if (!board.is_sq_attacked(E8, WHITE) && !board.is_sq_attacked(F8, WHITE) &&
                        !board.is_sq_attacked(G8, WHITE)) {
            ...
            if ((board.castle_perm() & BQC)) {
                ...
                if ((board.empty_bb() & bb) == bb) {
                    if (!board.is_sq_attacked(E8, WHITE) && !board.is_sq_attacked(D8, WHITE) &&
                        !board.is_sq_attacked(C8, WHITE)) {
